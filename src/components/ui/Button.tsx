...
export function NavyButtonLink({ children, href, fullWidth, className = '' }: { children: React.ReactNode; href: string; fullWidth?: boolean; className?: string }) {
  return (
    <a
      href={href}
      className={`inline-flex items-center justify-center gap-2 bg-navy-bolder text-white font-body font-bold text-sm tracking-[-0.3px] px-5 py-3.5 border border-navy-bolder hover:bg-navy transition-colors duration-150 ${fullWidth ? 'w-full' : ''} ${className}`}
    >
      {children}
    </a>
  )
